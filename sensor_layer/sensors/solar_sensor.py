...
class SolarSensor(BaseSensor):
    def __init__(self, home_id, dispatch_rate=5):
        ...
        # Solar is only producing between these hours.
        # 6am sunrise, 8pm it's basically dark.
        self.sunrise_hour = 6
        self.sunset_hour = 20

    def get_reading(self):
        now = datetime.now()
        hour = now.hour + now.minute / 60  # e.g. 14:30 becomes 14.5
        ...
        # At sunrise (hour 6) the angle is 0, at noon (hour 13) it peaks,
        # at sunset (hour 20) it's back to 0.
        # The formula maps the current hour into a 0й range.
