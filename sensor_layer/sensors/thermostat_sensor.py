...
class ThermostatSensor(BaseSensor):
    def __init__(self, home_id, dispatch_rate=5):
        ...
        # Start at a neutral temperature — the house might be at whatever temperature it was when we began the simulation:
    ...
    def get_reading(self):
        ...
        # Drift the current temperature toward the target. The further away we are from the target, the bigger the step:
        gap = target - self.current_temp
        self.current_temp += gap * self.drift_factor

        # Add a tiny bit of sensor noise — real thermistors have measurement variation. ±0.1°C is realistic for a decent smart thermostat sensor:
